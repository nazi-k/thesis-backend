...
async def create_guest_account() -> None:
    """Creates a guest account in the database."""
    await init_db.init()
    users = [
        User(
            name="Pickle Rick",
            email='rick@guest.api',
            avatarUrl='https://i.ibb.co/7JM1P2r/picke-rick.jpg',
            hashed_password=get_password_hash("pickle_rick")
        ),
        User(
            name="Baby Yoda",
            email='yoda@guest.api',
            avatarUrl='https://i.ibb.co/6n0hLML/baby-yoda.jpg',
            hashed_password=get_password_hash("baby_yoda")
        ),
        User(
            name="Lord Gaben",
            email='gaben@guest.api',
            avatarUrl='https://i.ibb.co/6RJ5hq6/gaben.jpg',
            hashed_password=get_password_hash("lord_gaben")
        )
    ]
    for user in users:
        await user.create()
    project = Project(
        name="Guest Project",
        description="This is a project created by the guest account.",
...
if __name__ == "__main__":
    asyncio.run(create_guest_account())
